...
const Profile = () => {
    ...
    return (
        <div className="w-screen bg-gradient-to-tr from-purple-900 to-indigo-900 h-screen flex flex-col items-center justify-start p-6">
            <div className=" w-[50vmax] flex bg-white rounded-lg shadow-lg overflow-hidden">
                <div className=" p-4 flex items-center justify-center bg-gray-200">
                    <img src={userProfile.profilePhoto || "path_to_default_image"} alt="Profile" className="rounded-full h-[10vmax] border border-red-300 bg-black w-[10vmax]" />
                </div>
                <div className="w-[80%] p-4">
                    <h2 className="text-2xl font-semibold mb-2">{userProfile.name}</h2>
                    <p className="text-gray-700 mb-1">Email: {userProfile.email}</p>
            ...
            <div className="friend-requests w-[50vmax] mt-6 p-4 bg-white rounded-lg shadow-md">
              <h3 className="text-[3vmax] font-semibold mb-4">Friend Requests</h3>
              {friendRequests.length === 0 ? (
                <p>No friend requests</p>
              ) : (
                friendRequests.map(request => (
                  <div key={request.email} className="user-card border bg-white border-gray-300
                   p-4 mb-4 flex items-center justify-between">
                    <div className="profile-pic mr-4">
                      <img src={request.profilePhoto || 'path_to_default_profile_pic'} alt="Profile Pic" className="w-[5vmax] h-[5vmax] bg-black rounded-full" />
                    </div>
                    <div className="flex-1">
                      <h2 className="text-[2vmax] font-semibold">{request.name}</h2>
                      <p className="text-gray-600">{request.email}</p>
                    </div>
                    <button
                      className="bg-blue-500 text-white px-[1vmin] py-[1vmin] rounded-md ml-auto"
                      onClick={() => acceptFriendRequest(request.email)}
                    >
                      Accept
                    </button>
